        ...
        <table className="w-full text-sm">
          <thead className="border-b border-neutral-900">
            <tr className="text-left font-mono text-[10px] uppercase tracking-widest text-neutral-500">
              ...
              <th className="py-3 font-medium">ROUND</th>
              <th className="py-3 font-medium">POT</th>
              <th className="py-3 font-medium">TICKETS</th>
              <th className="py-3 font-medium">BUYERS</th>
              <th className="py-3 font-medium">STATE</th>
              <th className="px-6 py-3 text-right font-medium">CLOSES / WINNER</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-900/60 font-mono text-[13px]">
            {ranked.map((p, i) => {
              ...
              return (
                <tr key={p.poolType} className="transition-colors hover:bg-neutral-900/40">
                  <td className="px-6 py-3 text-neutral-500">#{i + 1}</td>
                  <td className="py-3">
                  ...
                  <td className="py-3 text-neutral-500">#{p.round.toString()}</td>
                  <td className="py-3" style={{ color: accent }}>{potSol.toFixed(2)} SOL</td>
                  <td className="py-3 text-white">{tickets.toLocaleString()}</td>
                  <td className="py-3 text-neutral-400">{buyers}</td>
                  <td className="py-3">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] uppercase tracking-wider ${stateClass}`}>
                      {stateLabel}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-right text-neutral-400">{formatCloses(p)}</td>
                </tr>
              );
            })}
          </tbody>
