// Localnet pool init with **test-friendly** durations.
//
// The companion repo's scripts/init_public_pools.ts uses the spec's
// production durations (Sun Day 7, Wed Day 10, Fri Day 12, Sat Day 13 —
// all 20:00 UTC). Useful for prod, useless for testing the countdown UI
// because Weekly is always ~6 days away.
//
// This script bypasses that and inits each pool type with the durations a
// human can actually watch tick:
//
...
//
// Requires: validator running on localhost:8899, program deployed,
// ProtocolConfig already initialized (see scripts/reset-localnet.sh for
// the full bootstrap).
